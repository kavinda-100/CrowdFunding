...
contract DeployCrowdFundingFactory is Script {
    ...
    function run() external returns (CrowdFundingFactory) {
        ...
        // Write deployment summary to JSON file (local)
        // _writeDeploymentSummaryToJson(address(factory));

        // Write deployment summary to JSON file (Alchemy)
        _writeAlchemyDeploymentSummaryToJson(address(factory));
    ...
    function _writeAlchemyDeploymentSummaryToJson(address factoryAddress) private {
        // Step 1: Check if file exists and Step 2: Remove it if exists
        ...
        // Step 3: Create new JSON file with deployment summary
        string memory json = "alchemy_deployment_summary";
